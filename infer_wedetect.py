...
chinese_font = ImageFont.truetype('simsun.ttc', 20)
...
def visualize(output_file, image, bboxes, labels):
    ...
    for i in range(len(labels)):
        my_list.append(i)
    colors = [generate_random_color() for _ in my_list]
    ...
    for i in range(len(labels)):
        ...
        text_height = 20
        draw.rectangle([x1, y1, x1 + text_width, y1 + text_height], fill=colors[i])
...
def inference_detector(model,
    ...
    visualize(osp.join(output_dir, osp.basename(image_path)), image, pred_instances['bboxes'], labels)
